...
bool getSensorsRawUsb(rawSensors* out_sensors, const char* command)
{
    ...
    for (int attempt = 1; attempt <= NUM_USB_RETRIES; ++attempt) {
        ...
        ESP_LOGV(TAG, "getSensorsRawUSB: success on attempt %d", attempt);
        ESP_LOGV(TAG, "  Temperature: %u", temperature);
        ESP_LOGV(TAG, "  Accel: X=%d  Y=%d  Z=%d", ax, ay, az);
        ESP_LOGV(TAG, "  Mag:   X=%d  Y=%d  Z=%d", mx, my, mz);
        ESP_LOGV(TAG, "  Battery: %u mV", battery);
        return true; // success!
    }
